    ...
    * method='norm-sum': We normalize the map from 'sum' method within a cluster, i.e.
    ...
    if used_Tmap not in adata.uns["available_map"]:
        ...
        if (len(mega_cluster_list) == 0) or (np.sum(sp_idx) == 0):
            ...
            if horizontal:
                row = 1
                col = len(vector_array)
            else:
                row = len(vector_array)
                col = 1
    ...
    * fate_count=False: calculate the Shannon entropy of the fate probability
    ...
    if used_Tmap not in adata.uns["available_map"]:
        ...
        if (len(mega_cluster_list) == 0) or (np.sum(sp_idx) == 0):
            ...
            if background:
                customized_embedding(
                    x_emb, y_emb, np.zeros(len(y_emb)), point_size=point_size, ax=ax0
                )
            else:
                customized_embedding(
                    x_emb[cell_id_t1],
            ...
            if auto_color_scale:
                customized_embedding(
                    ...
                    set_lim=False,
                    color_bar=color_bar,
                    color_bar_label=colar_bar_label,
                )
            else:
                customized_embedding(
                    x_emb[cell_id_t1][sp_idx],
            ...
            if (len(mega_cluster_list) != 2) or (np.sum(sp_idx) == 0):
                ...
                if plot_target_state:
                    ...
                    for zz in valid_fate_list[0]:
                        ...
                        ax.plot(
                            x_emb[cell_id_t2[idx_2]],
                            y_emb[cell_id_t2[idx_2]],
                            ".",
                            color="red",
                    ...
                    customized_embedding(
                        x_emb[cell_id_t1_sp],
                        y_emb[cell_id_t1_sp],
                        np.zeros(len(y_emb[cell_id_t1_sp])),
                        point_size=point_size,
                        ax=ax,
                    )
...
def differential_genes(
    ...
    if group_A_idx is not None:
        ...
        if len(group_A_idx) != adata.shape[0]:
            logg.error(
                "group_A_idx should be a boolean array of the size adata.shape[0]."
            )
            return diff_gene_A, diff_gene_B
    ...
    if group_B_idx is not None:
        ...
        if len(group_B_idx) != adata.shape[0]:
            logg.error(
                "group_B_idx should be a boolean array of the size adata.shape[0]."
            )
            return diff_gene_A, diff_gene_B
    ...
    if group_A_idx is None:
        if "cell_group_A" not in adata.obs.keys():
            logg.error(
                "Cell population A not defined yet. Please define it directly at group_A_idx or adata.obs['cell_group_A']"
            )
            return diff_gene_A, diff_gene_B
        else:
    ...
    if group_B_idx is None:
        if "cell_group_B" not in adata.obs.keys():
            logg.error(
                "Cell population B not defined yet. Please define it directly at group_B_idx or adata.obs['cell_group_B']"
            )
            return diff_gene_A, diff_gene_B
        else:
    ...
    if (np.sum(group_A_idx) == 0) or (np.sum(group_B_idx) == 0):
        ...
        if plot_groups:
            ...
            customized_embedding(
                x_emb, y_emb, group_A_idx, ax=ax, point_size=point_size
            )
            ax.set_title(f"Group A")
            ax.axis("off")
            ax = plt.subplot(nrow, ncol, 2)
            customized_embedding(
                x_emb, y_emb, group_B_idx, ax=ax, point_size=point_size
            )
        ...
        if plot_gene_N > 0:
            ...
            fig, nrow, ncol = start_subplot_figure(
                plot_gene_N_A, row_height=2.5, n_columns=5, fig_width=16
            )
            ...
            fig, nrow, ncol = start_subplot_figure(
                plot_gene_N_B, row_height=2.5, n_columns=5, fig_width=16
            )
...
def differential_genes_for_given_fates(
    adata,
    selected_fates=None,
    selected_times=None,
    plot_groups=True,
    ...
    if (len(selected_fates) == 0) or (len(selected_fates) > 2) or (np.sum(sp_idx) == 0):
        logg.error(
            f"There should be only one or two fate clusters; or no cell states selected. Abort computation!"
        )
    else:
        ...
        if len(mega_cluster_list) == 0:
            ...
            diff_gene_A, diff_gene_B = differential_genes(
                adata,
                plot_groups=plot_groups,
                plot_gene_N=plot_gene_N,
...
def dynamic_trajectory_via_iterative_mapping(
    adata,
    selected_fate=None,
    used_Tmap="transition_map",
    map_backward=True,
    ...
    if used_Tmap not in adata.uns["available_map"]:
        ...
        resol = 10 ** (-10)
        used_map_0 = hf.sparse_rowwise_multiply(
            used_map_0, 1 / (resol + np.sum(used_map_0, 1).A.flatten())
        )
            ...
            if plot_separately:
                ...
                if apply_time_constaint:
                    customized_embedding(
                        ...
                        ax=ax0,
                        point_size=point_size,
                        title=f"Initial: t={sort_time_info[0]}",
                    )
...
def clonal_fate_bias(
    adata, selected_fate="", show_histogram=True, FDR=0.05, alternative="two-sided"
):
    """
    ...
    if len(mega_cluster_list) == 0:
        ...
        ax = plt.subplot(1, 1, 1)
        ax.plot(
            np.arange(len(fate_bias)), fate_bias, ".", color="blue", markersize=5
        ...
        result = pd.DataFrame(
            {
                "Clone ID": sort_idx,
                "Clone size": clone_size_array[sort_idx],
                "Q_value": P_value,
                "Fate bias": fate_bias,
            }
        )
    ...
    if len(mega_cluster_list) == 0:
        ...
        if (not os.path.exists(file_name)) or compute_new:
            ...
            for m in range(clone_N):
                ...
                if target_clone_size > 0:
                    target_ratio = (
                        np.sum(target_idx[target_cell_idx]) / target_clone_size
                    )
                    target_ratio_array[m] = target_ratio
                    # N_resampling=int(np.floor(cell_N/target_clone_size))
                    ...
                    for j in range(N_resampling):
                        temp_id = np.random.choice(
                            sel_cell_id_copy, size=target_clone_size, replace=False
                        )
                        null_ratio_array[m, j] = (
                            np.sum(target_idx[temp_id]) / target_clone_size
                        )

                    ## reprogamming clone
                    P_value_up[m] = (
                        np.sum(null_ratio_array[m] >= target_ratio) / N_resampling
                    )
                    P_value_down[m] = (
                        np.sum(null_ratio_array[m] <= target_ratio) / N_resampling
                    )
                    ...
                    for j1, zz in enumerate(null_ratio_array[m]):
                        P_value_up_rsp = (
                            np.sum(null_ratio_array[m] >= zz) / N_resampling
                        )
                        P_value_down_rsp = (
                            np.sum(null_ratio_array[m] <= zz) / N_resampling
                        )
        ...
        ax = plt.subplot(1, 1, 1)
        ax.plot(
            np.arange(len(fate_bias))[~idx],
            fate_bias[~idx],
            ".",
            color="blue",
            markersize=5,
            label=f"Size $<$ {int(clone_size_thresh)}",
        )  # ,markeredgecolor='black',markeredgewidth=0.2)
        ax.plot(
            np.arange(len(fate_bias))[idx],
            fate_bias[idx],
            ...
            color="red",
            markersize=5,
            label=f"Size $\ge$ {int(clone_size_thresh)}",
        )  # ,markeredgecolor='black',markeredgewidth=0.2)
        ax.plot(
            np.arange(len(fate_bias_rsp)),
            fate_bias_rsp,
            ".",
            color="grey",
            markersize=5,
        ...
        if show_histogram:
            target_fraction_array = (
                clonal_matrix_new.T * target_idx
            ) / clone_size_array
            fig = plt.figure(figsize=(fig_width, fig_height))
            ax = plt.subplot(1, 1, 1)
            ax.hist(target_fraction_array, color="#2ca02c", density=True)
...
def heatmap(
    figure_path, X, variable_names, color_bar_label="cov", data_des="", color_bar=True
):
    """
    ...
    plt.xticks(np.arange(X.shape[0]) + 0.4, variable_names, ha="right", rotation=60)
    plt.yticks(np.arange(X.shape[0]) + 0.4, variable_names)
    if color_bar:
        cbar = plt.colorbar()
...
def fate_hierarchy_from_Tmap(
    ...
    parent_map, node_groups, history = build_hierarchy_from_Tmap(
        adata,
        selected_fates=selected_fates,
        used_Tmap=used_Tmap,
        selected_times=selected_times,
...
def fate_hierarchy_from_clones(
    ...
    parent_map, node_groups, history = build_hierarchy_from_clones(
        adata,
        selected_fates=selected_fates,
        selected_times=selected_times,
        method=method,
...
def build_hierarchy_from_clones(
    ...
    return (
        parent_map,
        node_groups,
        (X_history, merged_pairs_history, node_names_history),
    )


def build_hierarchy_from_Tmap(
    adata,
    selected_fates=None,
    used_Tmap="transition_map",
    selected_times=None,
...
def plot_neighbor_joining(
    ...
    for i, X in enumerate(X_history):
        ...
        column_labels = [
            " + ".join([celltype_names[n] for n in grp]) for grp in column_groups
        ]
